...
import { load_dynamic_object, save_dynamic_object } from "../utilities.js";

function is_unlocked(id, completed) {
	if (completed.has(id)) return true  // the quest itself is completed
	const quest = quests[id]
	if ('require' in quest) return completed.has(quest.require)
	if ('require_all' in quest) return quest.require_all.every(ach => completed.has(ach))
	if ('require_any' in quest) return quest.require_any.some(ach => completed.has(ach))
	return true // when it doesn't have requirements
}

export function complete(player, id) {
  const completed = load_dynamic_object(player, "completed_achs", '[]')  // load
  if (completed.includes(id)) return  // skip if completed
  completed.push(id)  // add
  save_dynamic_object(player, "completed_achs", completed)  // save
...
export function undo(player, id) {
  let completed_achs = load_dynamic_object(player, "completed_achs", '[]')  // load
  completed_achs = completed_achs.filter(i => i != id)  // remove
  save_dynamic_object(player, "completed_achs", completed_achs)  // save
}
...
function claim(player, id) {
	...
	const claimed_rewards = load_dynamic_object(player, "claimed_rewards", '[]')  // load
	claimed_rewards.push(id)  // add
	save_dynamic_object(player, "claimed_rewards", claimed_rewards)  // save
}
...
world.afterEvents.worldLoad.subscribe(() => quest_tracker = load_dynamic_object(world, 'quest_tracker'))
...
system.runInterval(() => {  // detect quests automatically
	...
	world.getAllPlayers().forEach(player => {
		const completed = new Set(load_dynamic_object(player, "completed_achs", '[]'))
		Object.keys(quests)
		.filter(id => quests[id].query) // quest has a query
		.filter(id => !completed.has(id)) // quest isn't completed
		.filter(id => is_unlocked(id, completed)) // quest is unlocked
...
export function quests_menu(player, book) {
	...
	const completed_achs = load_dynamic_object(player, "completed_achs", '[]').reverse()
	const completed = new Set(completed_achs)
	const claimed_rewards = load_dynamic_object(player, "claimed_rewards", '[]')
	...
	.filter(id => id != '')
	.filter(id => view == "locked" ? !completed.has(id) : true)
	.filter(id => view == "done" ? completed.has(id) : true)
	.sort((a, b) => view == "done" ? completed_achs.indexOf(a) - completed_achs.indexOf(b) : 0)
	.filter(id => see_locked || is_unlocked(id, completed))
	
	listed_quests.forEach(id => {
		const {title, icon, reward} = quests[id]
		const is_done = completed.has(id)
		const is_locked = !is_unlocked(id, completed)
...
export function quest_screen(player, id, book) {
	const {title, icon, summary, reward, lines, notes, consume, image, checkmark, query, challenge, format} = quests[id]
	const completed = new Set(load_dynamic_object(player, "completed_achs", '[]'))
	const is_done = completed.has(id)
	const is_claimed = load_dynamic_object(player, "claimed_rewards", '[]').includes(id)
	const is_locked = !is_unlocked(id, completed)
